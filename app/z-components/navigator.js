...
export default function Navbar({scrolled = false }) {
  ...
  const links = [
    { name: "Αρχική", url: "/", businessOnly: false },
    { name: "Μενού", url: "/menu", businessOnly: false },
    { name: "Σχετικά", url: "/about", businessOnly: false },
    { name: "Παραγγελίες Live", url: "/live-orders", businessOnly: true },
    { name: "Επικοινωνία", url: "/contact", businessOnly: false, hideForBusiness: true },
  ];

  const sidebarLinks = [
    { name: "Προφίλ", url: "/profile", businessOnly: false },
    { name: "Παραγγελίες Live", url: "/live-orders", businessOnly: true },
    { name: "Όλες οι Παραγγελίες", url: "/all-orders", businessOnly: true },
    { name: "Μηνύματα", url: "/messages", businessOnly: true },
    { name: "Διαχείρηση Ωραρίου", url: "/schedule-manage", businessOnly: true },
    { name: "Οι Παραγγελίες μου", url: "/orders-history", businessOnly: false, hideForBusiness: true },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          ...
          <div className="absolute left-1/2 transform -translate-x-1/2 hidden md:flex space-x-8">
            {links.map((link) => {
              // Εμφανίζουμε τα businessOnly links μόνο αν business=true
              if (link.businessOnly && !business) return null;
              // Αν δεν είναι businessOnly ή αν business=true, το εμφανίζουμε
              if (link.hideForBusiness && business) return null;
              
              return (
                <Link key={link.url} href={link.url} className={linkClass}>
                  {link.name}
                </Link>
              );
            })}
        ...
        <div className="p-4 flex flex-col h-full">
          ...
          <ul className="flex-1 flex flex-col gap-3">
            {sidebarLinks.map((link) => {
              // Αν είναι businessOnly αλλά δεν έχουμε business, αγνοούμε
              if (link.businessOnly && !business) return null;

              // Αν είναι hideForBusiness και έχουμε business, αγνοούμε
              if (link.hideForBusiness && business) return null;
